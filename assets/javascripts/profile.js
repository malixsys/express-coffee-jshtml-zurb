  ...
  $(document).ready(function () {
      ...
      if (ab.val() == "") {
          //ab.val("@locals.lastValue('postalcode')");
      }
  });
  ...
  var marker_location = @ (locals.lastValue("location") ? "new google.maps.LatLng(" + locals.lastValue("location").latitude + ", " + locals.lastValue("location").longitude + ")" : false);

  function onStarted() {
      var mapOptions = {
          center: new google.maps.LatLng(@locals.lastValue('center').latitude, @locals.lastValue('center').longitude),
          zoom: 13,
          mapTypeId: google.maps.MapTypeId.ROADMAP
      };
      var map;
      window.map = map = new google.maps.Map(document.getElementById('map_canvas'),
      mapOptions);
      ...
      google.maps.event.addListener(window.autocomplete, 'place_changed', function () {
          ...
          infowindow.setContent('<div>"<strong>' + place.name + '</strong><br>" at ' + address);
